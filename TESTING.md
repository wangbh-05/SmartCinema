...
| 层级 | 结果 | 主要覆盖 |
| --- | ---: | --- |
| Node | 148/148 | v2 既有契约 + 商业目录、Money、票种/草稿、座位规则、报价、hold、订单/退款快照、Storage v3、迁移、恢复、应用与组合根 |
| 浏览器 | 14/14 | 新生产入口、票座一致、无障碍门控、锁座跨刷新恢复/冲突/释放、访客确认、幂等退票、响应式、Dialog、键盘、DOM 安全、运行时健康 |
...
13. `SEC-001`：用户字段只作为文本渲染；
14. `QA-001`：核心流程 0 个未处理 error/unhandledrejection。
...
二维码、真实支付与支付渠道退款回调、多日目录、内部工具拆分、真实读屏和完整辅助模式人工验收仍在商业路线图后续阶段。
