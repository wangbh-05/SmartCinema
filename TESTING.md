...
| 层级 | 结果 | 主要覆盖 |
| --- | ---: | --- |
| Node | 83/83 | v1/v2 迁移兼容 + 多电影/影院/日期目录、Money、票种/草稿、座位规则、报价、hold、订单/退款快照、辅助偏好、Storage v3、恢复、应用、运维权限、组合根与架构边界 |
| 浏览器 | 19/19 | 多目录切换、新生产入口、票座一致、无障碍门控、锁座跨刷新恢复/冲突/释放、访客确认、幂等退票、辅助偏好、响应式、Dialog、键盘、运维隔离/权限/人工释放、DOM 安全、运行时健康 |
...
14. `A11Y-002`：增强可读性、色觉友好和减少动态即时生效并跨刷新保存；
...
二维码、真实支付与支付渠道退款回调、真实读屏和完整辅助模式人工验收仍在商业路线图后续阶段。
