...
15. `ARCH-001`：v3 运维入口独立、禁止索引且不复用消费者/Canvas 组件树；
16. `OPS-001`：访客和普通会员无法读取内部运维数据；
17. `OPS-002`：管理员人工释放锁座必须二次确认，并原子更新 hold 与库存映射；
18. `QA-001`：核心流程 0 个未处理 error/unhandledrejection。
