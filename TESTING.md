...
> 当前事实日期：2026-07-19。生产入口与独立运维入口均使用商业领域 v3；v2 只保留数据迁移兼容契约，不再保留第二套 UI。
...
| 层级 | 结果 | 主要覆盖 |
| --- | ---: | --- |
| Node | 86/86 | v1/v2 迁移兼容 + 7 部影片/3 家影院/三日滚动排期、本地封面契约与 100/200/300 座目录、Money、1–20 张票/同行草稿、情侣隐私推荐、人群规则、四维座位决策、热度参考、报价、hold、订单/退款快照、辅助偏好、Storage v3、恢复、应用、运维权限、组合根与架构边界 |
| 浏览器 | 19/19 | 影片/影院横向滑窗、多目录/三厅切换、同行推荐、热度与体验辅助、票座一致、无障碍门控、锁座跨刷新恢复/冲突/释放、访客确认、幂等退票、四项辅助偏好、响应式、Dialog、键盘、运维隔离/权限/人工释放、DOM 安全、运行时健康 |
...
- [ ] 已停售场次不可选择；日期窗口始终保留本地当天及随后两天，即使当天场次已经全部停售；
- [ ] 每个票种的资格说明和价格清楚；
...
- `doc/PROJECT_REQUIREMENTS_RECONCILIATION.md`：商业产品与课程作业的冲突、缺口和推荐实现顺序；
