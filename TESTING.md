...
| 层级 | 结果 | 主要覆盖 |
| --- | ---: | --- |
| Node | 150/150 | v2 既有契约 + 商业目录、Money、票种/草稿、座位规则、报价、hold、订单/退款快照、辅助偏好、Storage v3、迁移、恢复、应用与组合根 |
| 浏览器 | 15/15 | 新生产入口、票座一致、无障碍门控、锁座跨刷新恢复/冲突/释放、访客确认、幂等退票、辅助偏好、响应式、Dialog、键盘、DOM 安全、运行时健康 |
...
9. `BUG-007`：登录/注册支持原生 submit/Enter；
10. `BUG-008`：内容拖出和背景点击均不关闭认证 Dialog；
11. `BUG-009`：关闭键、Escape、焦点归还；叠层中只关闭最上层；
12. `A11Y-001`：单一 Tab 停点、方向键、Space 和重绘后焦点保持；
13. `A11Y-002`：增强可读性、色觉友好和减少动态即时生效并跨刷新保存；
14. `SEC-001`：用户字段只作为文本渲染；
15. `QA-001`：核心流程 0 个未处理 error/unhandledrejection。
...
二维码、真实支付与支付渠道退款回调、多日目录、内部工具最终拆分、真实读屏和完整辅助模式人工验收仍在商业路线图后续阶段。
