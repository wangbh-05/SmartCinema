...
| 浏览器 | 13/13 | 新生产入口、票座一致、无障碍门控、锁座跨刷新恢复/释放、访客确认、幂等退票、响应式、Dialog、键盘、DOM 安全、运行时健康 |
...
8. `BUG-007`：登录/注册支持原生 submit/Enter；
9. `BUG-008`：内容拖出和背景点击均不关闭认证 Dialog；
10. `BUG-009`：关闭键、Escape、焦点归还；叠层中只关闭最上层；
11. `A11Y-001`：单一 Tab 停点、方向键、Space 和重绘后焦点保持；
12. `SEC-001`：用户字段只作为文本渲染；
13. `QA-001`：核心流程 0 个未处理 error/unhandledrejection。
...
并发冲突一键重选、二维码、真实支付与支付渠道退款回调、多日目录、内部工具拆分、真实读屏和完整辅助模式人工验收仍在商业路线图后续阶段。
