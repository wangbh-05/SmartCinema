...
\begin{frame}{分工与职责}
  \begin{table}[htbp]
    ...
    \begin{tabular}{p{0.18\textwidth}p{0.28\textwidth}p{0.38\textwidth}}
      ...
      成员 & 负责模块  \\
      \midrule
      王博翰 & 项目初始框架与开发流程设计、PPT制作  \\
      杨世威 & 项目第二版测试与优化 \\
      潘宇治 & 项目第三版后续开发、PPT制作  \\
      赵常景行 & 项目第四版后续开发、PPT制作  \\
      
      \bottomrule
    \end{tabular}
  \end{table}
  \begin{block}{协作方式（Git 工作流）}
    \begin{itemize}
      \setlength{\itemsep}{4pt}
      \item \textbf{版本管理}：Git + GitHub 远程仓库协作
        （\texttt{github.com/wangbh-05/SmartCinema}）
      \item \textbf{分支策略}：\texttt{main}（稳定版）维护可发布状态；\texttt{develop}（开发版）集成个人功能分支
      \item \textbf{合并流程}：个人功能分支开发 $\rightarrow$ 单元测试与代码评审（Pull Request）$\rightarrow$ 合并至 \texttt{develop} $\rightarrow$ 整体测试与优化后合入 \texttt{main}，并打版本标签标记稳定版本
      \item \textbf{例会制度}：每周例会简要汇报个人工作进展、遇到的问题与后续开发计划，明确本周里程碑
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{基础框架}
  % TODO: 讲解整体架构与模块分层（数据层 / 引擎层 / 表现层）、工程形态
  \begin{block}{待补充}
    （具体内容待团队补充）
  \end{block}
\end{frame}
...
\begin{frame}{核心技术选择}
  % TODO: 说明关键技术选型及选择理由（绘制 / 存储 / 工程形态 / 样式 / 无障碍等）
  \begin{block}{待补充}
    （具体内容待团队补充）
  \end{block}
\end{frame}
...
\begin{frame}{重点功能技术详解（一）}
  % TODO: 重点功能 1 技术详解（如 Canvas 弧形座位布局与坐标映射）
  \begin{block}{待补充}
    （具体内容待团队补充）
  \end{block}
\end{frame}

\begin{frame}{重点功能技术详解（二）}
  % TODO: 重点功能 2 技术详解（如智能推荐算法）
  \begin{block}{待补充}
    （具体内容待团队补充）
  \end{block}
\end{frame}

\begin{frame}{重点功能技术详解（三）}
  % TODO: 重点功能 3 技术详解（如观影体验评分 / 热度地图 / 订单中心，可按需增删帧）
  \begin{block}{待补充}
    （具体内容待团队补充）
  \end{block}
\end{frame}
