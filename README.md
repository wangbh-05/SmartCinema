...
- 3 部电影、2 家影院与未来 3 个营业日可组合导航；停售组合会禁用或自动落到最近可售日期；
...
本地演示目录使用 3 部虚构影片、2 家上海影院和 24 个三日场次；演示库存按场次与座位稳定生成，不依赖网络或随机运行时状态。
...
仍在后续阶段中的能力：真实支付与支付渠道退款回调、二维码、完整账户资料、真实读屏和完整辅助模式人工验收。v2 validator、迁移 fixture 与旧订单模型只承担历史数据兼容，不构成第二套产品 UI。
