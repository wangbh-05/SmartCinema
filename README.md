...
- 成人/儿童/学生/长者票步进器，单笔限制 1–20 张；2–4 名普通观众默认使用朋友同行，5 人起默认使用团体观影；
- 票数必须与座位数严格一致；
- 小/中/大三种目录影厅分别提供 100、200、300 个弧形 Canvas 座位，并同步隐藏 DOM 语义层供键盘与屏幕阅读器使用；
- “智能选座”使用票种、同行方式和靠中/靠后/过道偏好生成最多 5 组可切换方案；多人同行在同排连座不足时可推荐相邻两排的紧凑布局；儿童默认避开前三排、长者默认避开后三排，只有当前场次不存在任何合规布局时才分级放宽，手动选座仅提示不拦截；“需要无台阶通行”始终作为硬需求，无完整布局时会建议同日相邻场次；
- 双 Canvas 将座位与连续平面热力场分层绘制；热度支持周一至周日和一周综合即时切换，选择后显示视野、距离、周边空位与价格平衡四维说明；
...
- Canvas 座位图使用虚拟焦点：Tab 进入一次，方向键逐座移动，Space 选择；隐藏语义按钮层同步全部座位状态；
...
- 高频选座和键盘移动不使用位移动画，hover 仅对精确指针启用；首次 tooltip 轻延迟，连续浏览即时显示；
- 鼠标拖拽可框选普通可用座位；触摸设备保留自然横向滚动，避免手势冲突；
- 大字体会重新计算 Canvas 布局，高对比与色觉友好会增强轮廓/纹理，减少动态会关闭热度切换位移与缩放；
