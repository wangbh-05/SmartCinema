...
首次启动会直接创建当前状态，后续启动验证并复用同一状态。检测到低于当前版本的开发期状态时，应用会自动清理该 key 并重新初始化，无需手动清除浏览器缓存；损坏数据或更高版本状态仍会保留并停止启动，避免静默覆盖。恢复备份前会在 `smartcinema_import_rollback` 保存回滚快照；备份格式版本用于验证兼容性，不参与运行时迁移。
