...
| 1. 回归保护 | **下一阶段** | 为已知问题建立自动化或稳定的集成复现 | 新测试、测试矩阵 |
| 2. 架构设计 | 待开始 | 明确领域模型、依赖方向和存储 v2 | 架构 RFC、数据契约 |
| 3. 结构重构 | 待开始 | 按分层目标迁移代码且保持行为可验证 | 模块边界、测试结果 |
...
- [ ] 为放映日期库存隔离建立 Storage/Order 回归测试；
- [ ] 为订单用户隔离与未登录操作建立权限测试；
- [ ] 为支付重复点击建立幂等测试；
- [ ] 为本地选择与远端占座隔离建立状态测试；
- [ ] 为文本输入中的撤销快捷键建立 DOM 集成测试；
- [ ] 为登录回车、Escape、遮罩拖拽建立 Modal 交互测试；
- [ ] 为 320、390、768、800、900、1024、1440px 建立响应式检查；
- [ ] 将新增测试接入 `tests/runner.js` 或明确的浏览器测试入口。
