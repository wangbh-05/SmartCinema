...
| 4. 永久修复 | **已完成（13/13）** | 在新架构内关闭状态、订单、响应式与安全渲染类 Bug | 回归测试全绿 |
| 5. 交互基础重构 | **已完成** | 统一 Modal、Toast、表单、Canvas 和焦点行为 | 键盘/指针测试 |
| 6. 视觉与动效重构 | **已完成** | 建立设计系统、响应式布局和克制动效 | Before/After 审查、视觉验收 |
| 7. 全矩阵验收 | **已完成** | 覆盖桌面、移动、触控、键盘和辅助模式 | QA 报告 |
| 8. 收尾审计 | **已完成** | 清理遗留代码并补齐所有文档 | 完成审计清单 |
...
- [x] `BUG-006` 900px 以下网格产生隐式第二列、横向溢出并裁切窄屏 Canvas 座位；
...
- [x] 删除死代码、重复样式和过时文档；
- [x] 更新 README 的真实架构、命令、功能与限制；
- [x] 更新 TESTING 的自动化和浏览器验证方式；
- [x] 补充 LocalStorage schema、迁移和 v2 导入/导出恢复说明；
- [x] 补充无障碍、触控和 reduced-motion 说明；
- [x] 完成目标逐条证据审计；
- [x] 仅在全部目标被当前证据证明后标记长期 Goal 完成。
...
### 2026-07-18 · 阶段 4–6 · 永久修复与视觉重构

- 新建 `variables.css` 设计 tokens 与单一 `app.css` 页面系统，统一颜色、表面、文字、边框、间距、圆角、阴影、层级、Button、Input、Card、Modal、Toast、Badge 和响应式状态；删除已无引用的 `layout.css`、`components.css`、`responsive.css`；
- 首页移除约 340 行内联样式并重建语义页面骨架；桌面采用“座位与热度主列 + 推荐/评分/订单侧列”，1120px 以下退化为单列，320/390px 头部、工具栏、表单和设置控件改为适合触控的网格；
- BUG-006 从 XFAIL 转为正式回归；不仅检查 320、390、768、800、900、1024、1440px 页面 `scrollWidth`，还检查 Canvas 首末座位都位于绘制边界内，避免用 `overflow: hidden` 掩盖内部裁切；
- 座位布局按实际容器动态收缩 pitch、座位尺寸和上下边距；窄屏图例改为两行紧凑布局，320/390px 视觉矩阵确认 20 列座位、状态图例与影厅信息均完整可见；
- 登录/注册 Dialog 保留明确关闭按钮、Escape、焦点陷阱与焦点归还；内容拖到遮罩释放不会误关闭，且表单已有未提交内容时 backdrop 点击不再丢失输入，显式关闭后会清理密码与表单状态；
- 结算页拆为 `order.html`、`src/order.js` 与 `order.css`，复用设计 tokens，以安全 DOM 构造取代整页 `innerHTML` 模板；修复重复按钮文案并保留首次提交即禁用与幂等确认；
- Heatmap 移入 `ui/canvas/HeatmapRenderer`，配色降低饱和度并支持色盲模式；Cinema 点击反馈从 350ms 收敛为 240ms，`prefers-reduced-motion` 下不运行 Canvas 弹性动画；
- AI 顾问移除 200–600ms 随机假延迟，回复按发送顺序立即呈现；高频输入、键盘操作不添加动画，Hover 仅在精确指针设备启用，Dialog/Chat 进入动效为 180/160ms；
- 新增 `tests/visual-review.html`，并排渲染 320px 与 390px 生产首页供人工视觉验收；Node 实测 104/104，浏览器缺陷矩阵实测 PASS 11、XFAIL/XPASS/ERROR 均为 0。

#### Before / After / Why

| Before | After | Why |
| --- | --- | --- |
| 800px 以下出现隐式第二列，部分卡片移出视口 | 所有目标宽度使用显式单列，页面真实无横向溢出 | 响应式是布局约束，不应依赖裁切隐藏错误 |
| 320/390px 页面不溢出但 Canvas 只显示中间座位 | 动态 pitch 让首末座位、图例和影厅信息完整可见 | 自动 scrollWidth 检查不足以证明可用性，视觉内容也必须完整 |
| 首页由三份互相覆盖的旧 CSS 与大量内联规则驱动 | 单一 tokens + 页面系统，组件状态和断点集中管理 | 减少级联冲突，让视觉默认值可预测、可维护 |
| 订单页独立使用高饱和旧主题和字符串 HTML | 与首页共享视觉语言并使用安全 DOM 构造 | 核心流程跨页必须保持信任感、语义与安全边界一致 |
| 随机聊天延迟、350ms Canvas 弹性及无区分 Hover | 即时有序回复、240ms 可关闭反馈、指针能力限定 | 动效服务反馈而非制造等待，并尊重 reduced-motion 与触控设备 |

阶段 5、6 退出门槛已满足；当前进入阶段 7 全矩阵验收，尚未完成长期 Goal。

### 2026-07-18 · 阶段 7–8 · 全矩阵验收与收尾

- Node 最终实测 105/105 通过；浏览器最终实测 PASS 12（11 个缺陷契约 + QA-001 运行时健康检查），XFAIL/XPASS/ERROR 均为 0；
- 响应式契约在 320、390、768、800、900、1024、1440px 的默认与无障碍模式下运行；另验证 320px 大厅完整 30 列和 400% 缩放只在 Canvas 容器内滚动；
- 新增 100%–400% Canvas 缩放与窄屏触控提示：适应宽度用于总览，放大后可横向滑动进行更精确的手动点选；
- 完成人工“注册 → 推荐 → 应用 → 结算”和“取消结算 → 返回 → 历史订单为空”流程；复核亮色、深色、色盲、大字体与结算页；
- 浏览器 QA 汇总生产 iframe 的 `error` 和 `unhandledrejection`，核心交互期间未发现未处理异常；
- 安全备份与完整备份拆为两个明确动作；完整备份警告中的取消现在真正取消，不会继续下载另一种备份；
- resize 重绘按 animation frame 合并；显式平滑滚动也遵循 reduced-motion；
- 重写根 README、TESTING 与回归矩阵；为仍保留的 v1 报告、旧路线图和旧实现说明增加历史归档标记，防止继续被当作当前事实；
- 完成 `doc/QA_REPORT_2026-07-18.md`，记录自动化、人工流程、边界安全、动效审计与已知限制；
- 最终边界扫描确认 domain/application/shared 无 DOM、Canvas、Storage、系统时间或随机源，app/order/ui 无直接浏览器存储和字符串 `innerHTML`，生产 HTML 无内联 style/onclick；
- 阶段 7、8 退出门槛满足，长期 Goal 完成。
